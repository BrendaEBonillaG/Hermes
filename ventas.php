...
$sql = "SELECT 
    ...
    Compra.cantidad,
    Compra.fechaIngreso,
    u.nombreUsu AS nombre_comprador,
    GROUP_CONCAT(ip.url_imagen) AS imagenes_producto
FROM productos
INNER JOIN Compra ON productos.id = Compra.id_producto
INNER JOIN usuarios u ON Compra.id_comprador = u.id
LEFT JOIN Imagenes_Productos ip ON productos.id = ip.id_producto
WHERE productos.id_vendedor = ?
GROUP BY Compra.id";
...
if (!empty($compra['Imagenes_Productos'])) {
    ...
    echo '</div>';
                        ...
                        echo '<div class="info">';
                        echo '<h3>' . htmlspecialchars($compra['nombre']) . '</h3>';
